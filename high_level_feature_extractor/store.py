...
def raw_crowd_2_raw_crowd_HLF_table_format(
    ...
    raw_crowd_agged:pd.DataFrame = aggregate_crowd(
        df=raw_crowd, 
        aggregated_col_name=agg_col_name,
    )

    raw_crowd_unique_hashes:pd.DataFrame = raw_crowd_agged[~raw_crowd_agged.index.duplicated()]
...
def HLF_withspeaker_emottions_table(
    ...
    HLF_with_speaker_emotions:pd.DataFrame = HLF_table.join(
